...
Run the bundled script, replacing `<skill>` with this skill's installed directory.
It exists so the substitution is done the same way every time —
...
They are **exported copies** of the templates inside the [Primer CLI](https://github.com/QuinntyneBrown/primer)
...
The source and export script live in the Primer repository, not in Agent Toolkit or
the installed skill. Run these commands from the root of a Primer checkout:
...
Then copy both exported files from Primer's
`.claude/skills/agent-instruction-files/assets/` into this skill's `assets/` folder.
The bundled assets are sufficient for normal generation; a Primer checkout is only
needed to regenerate them from their source.
